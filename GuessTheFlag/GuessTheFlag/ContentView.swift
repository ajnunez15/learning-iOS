...
//  GuessTheFlag
//
//  Created by Andy Nunez on 1/8/22.
//
...
struct ContentView: View {
    ...
    @State private var userAnswer: Int = -1
    @State private var numOfQuestionsAsked = 0
    private var numOfQuestionsToAsk = 8
    ...
    var alertMessage: Text {
        if userAnswer > -1 && userAnswer != correctAnswer {
            return Text("Wrong! That’s the flag of \(countries[userAnswer])")
        }
    ...
    func askQuestion() {
        ...
        if numOfQuestionsAsked == numOfQuestionsToAsk {
            finishedGame = true
        }
    }
